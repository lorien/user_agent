...
def test_feature_popularity():
    ...
    for os, val in stats['os'].items():
        assert (float(val) / base.PLATFORM_POPULARITY['desktop'][os]) > 0.99
    for nav, val in stats['nav'].items():
        assert (float(val) / base.NAVIGATOR_POPULARITY['desktop'][nav]) > 0.99
