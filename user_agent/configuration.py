...
from voluptuous import Schema, Required, Length, All, ExactSequence, MultipleInvalid, SchemaError
...
class configuration(object):
    ...
    @staticmethod
    def load_external_configuration(external_config):
        """
           Load json from external_config string, validate and convert it,
           set parameters to config object

           :param external_config: values for versions of OS_PLATFORM, FIREFOX_VERSION, CHROME_BUILD, IE_VERSION and MACOSX_CHROME_BUILD_RANGE
           :type external_config: string in json format
        """

        external_config = json.loads(external_config)

        configuration._validate_external_configuration(external_config)

        configuration.OS_PLATFORM = external_config['OS_PLATFORM']
        configuration.FIREFOX_VERSION = configuration._list_to_tuple(external_config['FIREFOX_VERSION'])
        configuration.CHROME_BUILD = external_config['CHROME_BUILD']
        configuration.IE_VERSION = external_config['IE_VERSION']
        configuration.MACOSX_CHROME_BUILD_RANGE = external_config['MACOSX_CHROME_BUILD_RANGE']

    @staticmethod
    def load_external_file_configuration(json_file_path):
        with open(json_file_path, encoding='utf-8') as data_file:
            configuration.load_external_configuration(data_file.read())

    # --- Convertation functions ---

    @staticmethod
    def _date_hook(value):
        try:
    ...
    @staticmethod
    def _list_to_tuple(list_of_lists):
        return tuple(tuple(list(map(configuration._date_hook, x))) for x in list_of_lists)

    # --- Validation functions ---

    def _date(fmt=DATE_FORMAT):
        return lambda v: datetime.strptime(v, fmt)

    schema = Schema({
        Required('OS_PLATFORM'): Schema({
            Required('win'): All([All(str, Length(min=1))], Length(min=1)),
            Required('mac'): All([All(str, Length(min=1))], Length(min=1)),
            Required('linux'): All([All(str, Length(min=1))], Length(min=1)),
            Required('android'): All([All(str, Length(min=1))], Length(min=1))
        }),
        Required('FIREFOX_VERSION'): All([ExactSequence([str, _date()])], Length(min=1)),
        Required('CHROME_BUILD'): All([All([int], Length(min=3, max=3))], Length(min=1)),
        Required('IE_VERSION'): All([ExactSequence([int, str, str])], Length(min=1)),
        Required('MACOSX_CHROME_BUILD_RANGE'): Schema({str: All(ExactSequence([int, int]), Length(min=2, max=2))})
    })

    @staticmethod
    def _validate_external_configuration(external_config):
        # Validation according to schema
        configuration.schema(external_config)

        # External validation:

        # Length FIREFOX_VERSION and IE_VERSION lists
        for key, value in {'FIREFOX_VERSION': 2, 'IE_VERSION': 3}.items():
            for a in external_config[key]:
                try:
                    assert len(a) == value
                except AssertionError:
                    raise MultipleInvalid(errors=[
                    SchemaError("Too much values in {}".format(key))
                ])

        # MACOSX_CHROME_BUILD_RANGE versions should be present in OS_PLATFORM
        mac_platforms = list(
            map(lambda platform_string: platform_string.split('OS X ')[1], external_config['OS_PLATFORM']['mac']))
        for k in external_config['MACOSX_CHROME_BUILD_RANGE'].keys():
            try:
                assert k in mac_platforms
            except AssertionError:
                raise MultipleInvalid(errors=[
                    SchemaError("There's no {} version of mac OSX chrome build in OS_PLATFORM.mac".format(k))
                ])

        # Length of range of MACOSX_CHROME_BUILD
        for v in external_config['MACOSX_CHROME_BUILD_RANGE'].values():
            try:
                assert len(v) ==2
            except AssertionError:
                raise MultipleInvalid(errors=[
                    SchemaError("Too much values in MACOSX_CHROME_BUILD_RANGE field, in list {}".format(v))
                ])
